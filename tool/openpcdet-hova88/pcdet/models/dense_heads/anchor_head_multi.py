        ...
        if self.conv_dir_cls is not None:
            ...
            if self.use_multihead:
                dir_cls_preds = dir_cls_preds.view(
                    ...
                    -1, self.num_anchors_per_location, self.model_cfg.NUM_DIR_BINS, H, W).permute(0,  3, 4,1, 2).contiguous()
...
class AnchorHeadMulti(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        ...
        cnt = 0
        new_cls_output = []
        for one_head in ret['cls_preds']:
            one_head += 10
            new_one_head = torch.nn.functional.pad(one_head,(cnt,10-cnt-one_head.shape[4]))
            new_one_head -= 10
            cnt += one_head.shape[4]

            new_cls_output.append(new_one_head)

        cls_ret = torch.cat(new_cls_output,dim=3).view((1,128,128,20*10))
        box_ret = torch.cat(ret['box_preds'],dim=3).view((1,128,128,20*7))
        dir_ret = torch.cat(ret['dir_cls_preds'],dim=3).view((1,128,128,20*2))

        return (cls_ret,box_ret,dir_ret)
